...
export function setupEventiHandlers(db: Database) {
  ...
  // GET BY ID
  ipcMain.handle('eventi:getById', async (_, id: number) => {
    try {
      const result = db.prepare(`
        SELECT 
          e.*,
  ...
  // CREATE
  ipcMain.handle('eventi:create', async (_, evento: Omit<Evento, 'id'>) => {
    try {
      const validation = await ipcMain.handle('eventi:validate', null, evento);
      ...
      if (result.changes > 0) {
        const newEvento = await ipcMain.handle('eventi:getById', null, result.lastInsertRowid);
        return newEvento;
      }
  ...
  // UPDATE
  ipcMain.handle('eventi:update', async (_, id: number, eventoData: Partial<Evento>) => {
    try {
      const currentEvento = await ipcMain.handle('eventi:getById', null, id);
      ...
      const validation = await ipcMain.handle('eventi:validate', null, eventoToValidate);
      if (!validation.isValid) {
        return {
          success: false,
          error: 'Validazione fallita',
      ...
      if (result.changes > 0) {
        const updatedEvento = await ipcMain.handle('eventi:getById', null, id);
  ...
  // VALIDATE BULK
  ipcMain.handle('eventi:validateBulk', async (_, eventi: Evento[]) => {
    const results: { [key: string]: { isValid: boolean; errors: string[] } } = {};
