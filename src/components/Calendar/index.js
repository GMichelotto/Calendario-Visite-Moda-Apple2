...
const CalendarComponent: React.FC = () => {
  ...
  // Validazione evento
  const validateEvent = useCallback(async (eventData: any, excludeEventId: string | null = null) => {
    try {
  ...
  // Gestione drag & drop
  const moveEvent = useCallback(async ({ event, start, end }: { event: ICalendarEvent; start: Date; end: Date }) => {
    try {
      const eventData = {
        ...event,
        data_inizio: start,
  ...
  const resizeEvent = useCallback(async ({ event, start, end }: { event: ICalendarEvent; start: Date; end: Date }) => {
    try {
      const eventData = {
        ...event,
        data_inizio: start,
  ...
  const handleSelectEvent = useCallback(async (event: ICalendarEvent) => {
    try {
  ...
  const eventStyleGetter = useCallback((event: ICalendarEvent) => ({
    style: {
